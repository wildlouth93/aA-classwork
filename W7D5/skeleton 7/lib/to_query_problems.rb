...
def no_apples_for_blair_sub
  ...
  execute(<<-SQL)
    ...
    JOIN
      toys ON toys.id = cattoys.toy_id 
    WHERE
      cats.name != 'Blair' AND toys.name IN (
        SELECT
          name
        FROM
          toys
...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    FROM
      toys AS a
    JOIN
      toys AS b ON b.price = a.price 
    WHERE
      b.name = 'Shiny Mouse' AND a.name != 'Shiny Mouse' 
    ORDER BY 
      a.name; 
  SQL
end
...
def price_like_shiny_mouse_sub
  ...
  execute(<<-SQL) 
    ...
    FROM 
      toys 
    WHERE 
      name != 'Shiny Mouse' AND toys.price IN (
        SELECT 
          price 
        FROM 
          toys 
...
def just_like_orange
  ...
  execute(<<-SQL)
    ...
    FROM 
      cats AS a 
    JOIN 
      cats AS b ON b.breed = a.breed  
    WHERE 
      b.name = 'Orange' AND a.name != 'Orange' 
    ORDER BY 
      a.name;
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      Bcats.name, toys.name 
    FROM 
      toys 
    JOIN 
      cattoys AS Acattoys ON Acattoys.toy_id = toys.id 
    JOIN 
      cats AS Acats ON Acats.id = Acattoys.cat_id 
    JOIN 
      cattoys AS Bcattoys ON Bcattoys.toy_id = toys.id 
    JOIN 
      cats AS Bcats ON Bcats.id = Bcattoys.cat_id 
    WHERE 
      Acats.name = 'Jet' AND Bcats.name != 'Jet' 
    ORDER BY 
      Bcats.name; 
  SQL
end
